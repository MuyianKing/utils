...
- 定位：通用工具函数库，发布为 npm 包 `@muyianking/utils`（当前版本 `0.2.1`），**纯 ESM**、`sideEffects: false`、支持 tree-shaking。
...
- 构建：`@rslib/core` `^1.0.1`，经 `script/build.js` 调用 `rslib build`；只输出 ESM（`output.target: web`、`syntax: chrome >= 60`），类型声明输出到 `dist/types/`。
- 运行时依赖固定为 5 个：`@vueuse/core` `^15.0.0`、`dayjs` `^1.11.23`、`echarts` `^6.1.0`、`lodash-es` `^4.18.1`、`nanoid` `^6.0.1`。
- 工具链：`eslint` 精确锁定 `10.11.0`（无 `^`，扁平化配置）、`@antfu/eslint-config` `^9.5.1`、`eslint-plugin-format` `^2.0.1`、`typescript` `^6.0.3`、`vitest` `^5.0.1` 与 `@vitest/coverage-v8` `^5.0.1`、`jsdom` `^30.1.0`、`husky` `^9.1.7`、`lint-staged` `^17.5.1`、`@commitlint/cli` `^21.2.2`、`cz-git` `^1.14.0`。
...
- 产物约束：README 承诺类型声明在 `bundler` / `node16` / `nodenext` 下均可解析，已实测成立。rslib 1.0 会把源码里的 `.ts` 说明符**统一改写为 `.js`** 再写进 `dist/types/*.d.ts`（`@rslib/core` `0.22` 时代是原样保留 `.ts`），所以 `src/` 里写 `.ts` 还是 `.js` 对产物无影响，不构成兼容性差异（`node10` 在 TypeScript 6 下仍需 `ignoreDeprecations`，且该选项 TS 7 将停止支持）；改动相对导入扩展名或 rslib `dts` 配置后，重新构建并抽查 `dist/types/index.d.ts` 的相对说明符都能在 `dist/types/` 下找到同名 `.d.ts`。
...
- 依赖升级：用 `pnpm exec ncu` **分档**做——先 `--target minor`（同 major 内小版本）跑通门禁，再逐个 major 单独升级并复跑门禁，避免一次升完失败后无法归因。两个已踩过的坑：①`typescript` 必须停在 `^6.0.3`，升到 `7.x` 会让 `pnpm lint` 直接崩溃——typescript-eslint 明确不支持 TS 7.0（上游 `typescript-eslint#10940` 在跟踪 7.1+），要用 TS 7 只能按官方 side-by-side 方案让 typescript-eslint 走 TS 6 API；②pnpm 12 自带 24 小时最小发布年龄供应链策略，装到发布不足 24h 的包时 pnpm 会自动生成 `pnpm-workspace.yaml` 并写入 `minimumReleaseAgeExclude`，而该文件一旦存在，`@antfu/eslint-config` 的 `pnpm/yaml-enforce-settings` 就会要求补 `shellEmulator` / `trustPolicy` / `minimumReleaseAgeExcludePrune` 三项设置而报错——正解是把该包钉回旧版或等它过 24h，不要为此新增这份豁免文件（`@commitlint/cli` 因此停在 `^21.2.2`，仓库当前无 `pnpm-workspace.yaml`）。

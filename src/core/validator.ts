...
// vant-ui的校验回调
function vantCallback(data: Error | string): string {
...
type CallbackFun = (data?: Error | string) => string | boolean
type ValidateFun = (value: any) => boolean

/**
 * 表单校验方法生成器
 * @param rule 目前为element-plus校验中的校验规则，其他传null
 * @param value 校验的值
...
// 获取值
function getValue(rule: any, value: any, callback: CallbackFun) {
  // vant-ui取第一个参数
...
// 验证表单是整数
export function v_int(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isInt, callback, '只能是整数')
}

// 验证表单是车牌
export function v_carnum(rule: any, value: any, callback: CallbackFun) {
...
// 验证表单是端口号
export function v_port(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isPort, callback, '端口号格式错误')
}

// 验证表单是邮箱
export function v_email(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isEmail, callback, '邮箱地址格式错误')
}

// 验证表单手机号
export function v_phone(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isMobilePhone, callback, '手机号码格式错误')
}

// 验证座机
export function v_tel(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isTelphone, callback, '电话号码格式错误')
}

// 验证表单IP
export function v_ip(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isIP, callback, 'IP格式错误')
}

// 验证表单经纬度
export function v_latlong(lat: number | string, lon: number | string, callback: CallbackFun) {
  return validateService(null, `${lat},${lon}`, isLatLong, callback, '经纬度格式错误')
}

// 验证表单身份证
export function v_id_num(rule: any, value: any, callback: CallbackFun) {
  return validateService(rule, value, isIdcardAll, callback, '身份证格式错误')
}

// 验证手机号和座机号
export function v_phoneOrTel(rule: any, value: any, callback: CallbackFun) {

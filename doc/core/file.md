...
```typescript
const mime_type: Array<{ suffix: string; mime_type: string }>
```

后缀名到 MIME 类型的映射表，包含 65+ 种常见文件类型。
...
**返回**: `string | string[]` — 输入为字符串返回字符串，输入为数组返回数组
...
```typescript
function readBlobAsJSON(blob: Blob): Promise<any>
```
